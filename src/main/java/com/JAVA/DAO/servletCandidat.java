...
/**
 * Servlet implementation class servletCandidat
 */
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (action.equals("delete")) {
            ...
            if (nom != null && !nom.isEmpty()) {
                try {
                    CandidatDaoImpl test = new CandidatDaoImpl(DAOFactory.getInstance());
            ...
            this.getServletContext().getRequestDispatcher("/DeleteCandidatResponse.jsp").forward(request, response);
...
//    	            CandidatBean newUser = new CandidatBean(0, nom, prenom,email,password);
//
...
//    	    this.getServletContext().getRequestDispatcher("/DashboardCandidat.jsp").forward(request, response);
                ...
                // Vérifiez si les informations obligatoires sont présentes
                if (nom != null && prenom != null && email != null) {
                    try {
                        CandidatDaoImpl test = new CandidatDaoImpl(DAOFactory.getInstance());

                        // Créez un objet CandidatBean avec les informations de l'utilisateur
                        CandidatBean newUser = new CandidatBean(0, nom, prenom, email, password);
        		...
        		this.getServletContext().getRequestDispatcher("/FindCandidatResponse.jsp").forward( request, response );
...
//           
 	  ...
 	  }else if (action.equals("loginCandidat")) {
          ...
          if (email != null && password != null) {
              CandidatDaoImpl candidatDao = new CandidatDaoImpl(DAOFactory.getInstance());  // Vous devez obtenir cette instance d'une manière appropriée

              CandidatBean candidat = candidatDao.loginCandidat(email, password);
              ...
              if (candidat != null) {
                  ...
                  response.sendRedirect("DashboardCandidat.jsp"); // Redirigez vers la page d'accueil des recruteurs
              } else {
